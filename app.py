...
# --- 7.投资收益、承保损益单柱图 ---
    ...
    df_tax_pivot['有效税率'] = np.where(
        ...
        (df_tax_pivot['税前利润总额'] - df_tax_pivot['净利润']) / denom,  # ✅ 用denom不用原列
        0
    )
...
# 26. 附录：业绩明细表与新业务明细表 (双表同打)
            ...
            for pair_idx, (left_fig, right_fig) in enumerate(pairs):
                ...
                combined.update_layout(height=245 if print_mode else 320,
